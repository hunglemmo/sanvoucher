...
const RedemptionModal = ({ onClose, userCoins, showNotification, onRedemptionSuccess, backendUrl }) => {
    ...
    const cardOptions = [
        { value: 10000, label: '10,000đ', coins: 1000 },
        { value: 20000, label: '20,000đ', coins: 2000 },
        { value: 50000, label: '50,000đ', coins: 5000 },
        { value: 100000, label: '100,000đ', coins: 10000 },
    ];
    const handleSubmit = async (e) => {
        e.preventDefault();
        const requiredCoins = cardOptions.find(opt => opt.value === amount)?.coins || 0;
        if (userCoins < requiredCoins) {
            showNotification("Số xu của bạn không đủ.", 'error');
            return;
        }
        setIsLoading(true);
        try {
            ...
            onRedemptionSuccess(result.newCoins);
            onClose();
        } catch (error) {
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-40" onClick={onClose}>
            <div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-sm mx-4" onClick={e => e.stopPropagation()}>
                <h3 className="text-xl font-bold text-center mb-4">Đổi Thẻ Cào</h3>
                <p className="text-center text-sm text-gray-600 mb-4">Thẻ sẽ được trả trong mục "Lịch sử" trong vòng 24 giờ.</p>
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        <label className="block text-sm font-medium text-gray-700">Chọn nhà mạng</label>
                        <select value={cardType} onChange={e => setCardType(e.target.value)} className="mt-1 block w-full p-2 border border-gray-300 rounded-md shadow-sm">
                    ...
                    <div>
                        <label className="block text-sm font-medium text-gray-700">Chọn mệnh giá</label>
                        <select value={amount} onChange={e => setAmount(Number(e.target.value))} className="mt-1 block w-full p-2 border border-gray-300 rounded-md shadow-sm">
                            {cardOptions.map(opt => (
                                <option key={opt.value} value={opt.value}>{opt.label} (Cần {opt.coins} xu)</option>
                            ))}
                        </select>
                    </div>
                    <p className="text-xs text-gray-500 mt-1">Số dư của bạn: {userCoins} xu</p>
                    <button type="submit" className="w-full bg-green-500 hover:bg-green-600 text-white font-bold py-2 rounded-lg transition disabled:bg-gray-400" disabled={isLoading}>
                        {isLoading ? 'Đang gửi...' : 'Xác nhận đổi'}
                    </button>
                </form>
            </div>
        </div>
...
const RedemptionHistoryPage = ({ showNotification, backendUrl }) => {
    ...
    return (
        <div className="p-4">
            <h2 className="text-2xl font-bold mb-4 text-center">Lịch sử đổi thưởng</h2>
            {isLoading ? <p className="text-center">Đang tải...</p> : (
                <div className="space-y-4">
                    {history.length === 0 ? <p className="text-center text-gray-500">Bạn chưa có lịch sử đổi thưởng nào.</p> : history.map(item => (
                        <div key={item._id} className="bg-white p-4 rounded-lg shadow">
                            <div className="flex justify-between items-center mb-2">
                                <span className="font-bold">{item.cardType} - {formatCurrency(item.amount)}</span>
                                <span className={`px-2 py-1 text-xs font-semibold rounded-full ${item.status === 'completed' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}>
                                    {item.status === 'pending' ? 'Đang xử lý' : 'Thành công'}
                                </span>
                            </div>
                            <p className="text-sm text-gray-500 mb-2">Ngày: {new Date(item.createdAt).toLocaleString('vi-VN')}</p>
                            {item.status === 'completed' && item.cardCode && (
                                <div className="bg-gray-100 p-2 rounded mt-2">
                                    <p className="text-sm">Mã thẻ: <strong className="font-mono">{item.cardCode}</strong> <button onClick={() => copyToClipboard(item.cardCode)} className="text-blue-500 text-xs ml-2">[SAO CHÉP]</button></p>
                                    <p className="text-sm">Seri: <strong className="font-mono">{item.cardSerial}</strong> <button onClick={() => copyToClipboard(item.cardSerial)} className="text-blue-500 text-xs ml-2">[SAO CHÉP]</button></p>
                                </div>
                            )}
                        </div>
                    ))}
    ...
    return (
        <div className="min-h-screen bg-gray-100 flex flex-col justify-center items-center p-4">
            <div className="w-full max-w-sm bg-white p-8 rounded-2xl shadow-lg text-center">
                <h1 className="text-3xl font-bold text-gray-800 mb-2">{isLoginView ? 'Chào mừng!' : 'Tạo tài khoản'}</h1>
                <p className="text-gray-500 mb-8">{isLoginView ? 'Đăng nhập để săn sale và tích xu.' : 'Điền thông tin để bắt đầu.'}</p>
                <form onSubmit={(e) => { e.preventDefault(); handleAuthAction(); }}>
                    <div className="space-y-4">
                        <input type="text" placeholder="Tên đăng nhập" className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" value={username} onChange={e => setUsername(e.target.value)} required />
                        <input type="password" placeholder="Mật khẩu" className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" value={password} onChange={e => setPassword(e.target.value)} required />
                        {!isLoginView && (
                            <>
                                <input type="password" placeholder="Nhập lại mật khẩu" className="w-full px-4 py-3 border border-gray-300 rounded-lg" value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} required />
                                <input type="text" placeholder="Mã giới thiệu (nếu có)" value={referralCode} onChange={e => setReferralCode(e.target.value)} className="w-full px-4 py-3 border border-gray-300 rounded-lg" />
                            </>
                        )}
                    </div>
                    <button type="submit" className="w-full mt-6 bg-blue-500 hover:bg-blue-600 text-white font-bold py-3 px-4 rounded-lg transition-transform transform hover:scale-105 disabled:bg-gray-400" disabled={isLoading}>
                ...
                <div className="flex justify-center">
                    {Capacitor.isNativePlatform() ? (
                        <button type="button" onClick={handleNativeGoogleLogin} disabled={isLoading} className="flex items-center justify-center w-full py-2 px-4 border border-gray-300 rounded-lg shadow-sm bg-white text-sm font-medium text-gray-700 hover:bg-gray-50 disabled:opacity-50">
                            <GoogleIcon />
                            <span className="ml-3">Đăng nhập với Google</span>
                        </button>
                    ) : (
                        <GoogleLogin onSuccess={credentialResponse => handleGoogleSuccess(credentialResponse.credential)} onError={() => { showNotification('Đăng nhập Google thất bại.', 'error'); }} useOneTap />
                    )}
                </div>
                <p className="mt-8 text-sm">
                    {isLoginView ? 'Chưa có tài khoản? ' : 'Đã có tài khoản? '}
                    <button onClick={() => setIsLoginView(!isLoginView)} className="font-semibold text-blue-500 hover:underline">
...
const CategoryPage = ({ availablePlatforms, setPlatformFilter, setCurrentPage }) => {
    const allPlatforms = [
        { name: 'Shopee', logo: 'https://placehold.co/400x200/EE4D2D/FFFFFF?text=Shopee', filter: 'shopee' },
        { name: 'Lazada', logo: 'https://placehold.co/400x200/0F146D/FFFFFF?text=Lazada', filter: 'lazada' },
        { name: 'Tiki', logo: 'https://placehold.co/400x200/1A94FF/FFFFFF?text=Tiki', filter: 'tiki' },
    ];
                ...
                {displayPlatforms.length > 0 ? (
                    displayPlatforms.map(platform => (
                        <button key={platform.name} onClick={() => handlePlatformClick(platform.filter)} className="block bg-white p-4 rounded-lg shadow-md hover:shadow-lg transition">
                            <img src={platform.logo} alt={platform.name} className="w-full h-auto rounded-md object-contain aspect-[2/1]" />
                        </button>
                    ))
                ) : (
                    <p className="col-span-2 text-center text-gray-500">Không có chiến dịch nào từ các nền tảng.</p>
                )}
            </div>
        </div>
    );
    ...
    return (
        <div className="p-3 text-center">
            ...
            <div className="space-y-3">
                <button onClick={onDailyLogin} disabled={!canClaimDailyBonus} className="w-full bg-blue-500 text-white font-bold py-3 rounded-lg disabled:bg-gray-400 disabled:cursor-not-allowed">
                    {canClaimDailyBonus ? "Nhận thưởng đăng nhập (+10 xu)" : "Đã nhận thưởng hôm nay"}
                </button>
...
const HomePage = ({ products, isLoading, error, platformFilter, setPlatformFilter, loadMore, hasMore, isLoadingMore }) => {
    if (isLoading) { return <div className="p-4 text-center">Đang tải sản phẩm...</div>; }
    if (error) { return <div className="p-4 text-center text-red-500">{`Lỗi: ${error}. Hãy đảm bảo backend của bạn đang chạy.`}</div>; }
    const displayProducts = platformFilter ? products.filter(p => p.domain && p.domain.includes(platformFilter)) : products;
    if (displayProducts.length === 0 && !isLoading) {
        return (
            <div className="p-4 text-center text-gray-500 mt-10">
                <p>Không tìm thấy sản phẩm nào.</p>
                <p className="text-sm mt-2">Có thể API của AccessTrade tạm thời không có dữ liệu hoặc API key của bạn chưa có chiến dịch nào được duyệt.</p>
                {platformFilter && <button onClick={() => setPlatformFilter(null)} className="mt-4 text-blue-500 font-semibold">Bỏ lọc</button>}
            </div>
        );
    }
    return (
        <div className="bg-gray-100">
            ...
            <div className="p-3 mt-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                    {displayProducts.map(p => <ProductCard key={p.id || p.link} product={p} />)}
                </div>
                <div className="text-center mt-6">
                    {isLoadingMore && <p>Đang tải thêm...</p>}
                    {!isLoadingMore && hasMore && (
                        <button onClick={loadMore} className="bg-white border border-gray-300 text-gray-700 font-semibold py-2 px-6 rounded-full shadow-sm hover:bg-gray-50 transition">Xem thêm</button>
                    )}
                    {!hasMore && <p className="text-gray-500">Bạn đã xem hết sản phẩm.</p>}
                </div>
            </div>
        </div>
    );
...
export default function App() {
    ...
    const [isAppLoading, setIsAppLoading] = useState(true); // << THÊM MỚI: State để quản lý màn hình chờ

    // THÊM MỚI: useEffect để phục hồi phiên đăng nhập
    useEffect(() => {
            ...
            if (token) {
                try {
                    ...
                    if (response.ok) {
                        ...
                        if (data.success) {
                            // Dùng lại hàm handleLogin để cập nhật state
                            handleLogin({ token, user: data.user }, false); // false để không chuyển trang
                        } else {
                    ...
                    console.error("Lỗi phục hồi phiên:", err);
                    localStorage.removeItem('authToken');
                }
            }
            setIsAppLoading(false); // << Kết thúc màn hình chờ
        };
    ...
    useEffect(() => {
        if (currentPage === 'profile' && isAuthenticated) { // << Sửa lại điều kiện
            checkBonusStatus();
        }
    }, [currentPage, isAuthenticated]); // Thêm isAuthenticated để đảm bảo chỉ chạy khi đã đăng nhập
    ...
    // SỬA LẠI: Thêm tham số để kiểm soát việc chuyển trang
    const handleLogin = (data, shouldChangePage = true) => {
    ...
    // THÊM MỚI: Hiển thị màn hình chờ trong khi phục hồi phiên
    if (isAppLoading) {
        return <div className="min-h-screen flex items-center justify-center">Đang tải ứng dụng...</div>;
    }
